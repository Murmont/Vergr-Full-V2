...
export default function SquadEventsScreen() {
  ...
  useEffect(() => {
    if (!squadId) return;
    getDocs(query(collection(db, 'squads', squadId, 'events'), orderBy('createdAt', 'desc'), limit(20)))
    ...
    <div className="pb-8">
      {isMobile ? (
        <div className="sticky top-0 z-40 glass-header flex items-center gap-3 px-4 py-3 border-b border-white/[0.04]">
          ...
          <h1 className="text-white font-syne font-bold text-lg">Events</h1>
        </div>
      ) : (
      ...
      <div className="px-4 py-4">
        ...
        : items.length === 0 ? (
          <div className="text-center py-16">
            <Icon name="event" size={48} className="text-text-muted/30 mx-auto mb-3" />
            <p className="text-text-muted text-sm">No events scheduled</p>
            <p className="text-text-muted/60 text-xs mt-1">Events are organized by squad leadership</p>
          </div>
        ) : <div className="space-y-3">{items.map(item => (
          <div key={item.id} className="p-4 rounded-2xl bg-surface-1 border border-white/[0.06]">
            <h3 className="text-sm font-bold">{item.title || 'Event'}</h3>
            {item.date && <p className="text-xs text-brand-cyan font-dmmono mt-1">{new Date(item.date?.toDate ? item.date.toDate() : item.date).toLocaleDateString()}</p>}
